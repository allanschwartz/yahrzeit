...
static void  selftest_corners( byte panel )
{
    ...
    if ( panel == PANEL0 ) {
                // bit, row, col
        ledWall.storeInArray( 1, 1, 1 );
        ledWall.storeInArray( 1, NROWS, 1 );
        ledWall.storeInArray( 1, 1, NCOLS );
        ledWall.storeInArray( 1, NROWS, NCOLS );
    }
    else {
                        // bit, row, col, panel
        ledWall.storeInPanel( 1, 1, 1, panel);
        ledWall.storeInPanel( 1, ledWall.rowsInPanel(panel), 1, panel );
        ledWall.storeInPanel( 1, 1, ledWall.colsInPanel(panel), panel );
        ledWall.storeInPanel( 1, ledWall.rowsInPanel(panel), ledWall.colsInPanel(panel), panel );
    }
    sleep_ms( true, 500);
}
    ...
    if ( panel == PANEL0 ) {
        ...
        for ( byte col = 1; col <= NCOLS; col++ ) {
            for ( byte row = 1; row <= NROWS; row++ ) {
                int rc = ledWall.storeInArray( pixelbit, row, col );
                ASSERT(rc == NO_ERROR);
            }
        }     
        sleep_ms( true, 1);
        ...
        for ( byte col = 1; col <= ledWall.colsInPanel(panel); col++ ) {
            for ( byte row = 1; row <= ledWall.rowsInPanel(panel); row++ ) {
                int rc = ledWall.storeInPanel( pixelbit, row, col, panel );
                ASSERT(rc == NO_ERROR);
            }
        }
        sleep_ms( true, 1);
...
//        for ( byte row = 1; row <= NROWS; row++ ) {
//            for ( byte col = 1; col <= NCOLS; col++ ) {
//                ledWall.storeInArray( 1, row, col );
//            }
//            sleep_ms( true, 200 );
//
//            for ( byte col = 1; col <= NCOLS; col++ ) {
//                ledWall.storeInArray( 0, row, col );
//            }
//            sleep_ms( true, 10 );
//        }
//    }
//    else {
        for ( byte row = 1; row <= ledWall.rowsInPanel(panel); row++ ) {
            for ( byte col = 1; col <= ledWall.colsInPanel(panel); col++ ) {
                int rc = ledWall.storeInPanel( 1, row, col, panel );
                ASSERT(rc == NO_ERROR);
            }
            sleep_ms( true, 300 );

            for ( byte col = 1; col <= ledWall.colsInPanel(panel); col++ ) {
                int rc = ledWall.storeInPanel( 0, row, col, panel );
                ASSERT(rc == NO_ERROR);
            }
            sleep_ms( true, 1 );
        }
...
static void  selftest_cylon( byte panel )
{
    ...
    for ( byte col = 1; col <= ledWall.colsInPanel(panel); col++ ) {
        for ( byte row = 1; row <= (ledWall.rowsInPanel(panel) - 1); row++ ) {
            ledWall.storeInPanel( 1, row, col, panel );
            ledWall.storeInPanel( 1, row+1, col, panel );
            sleep_ms( true, 50 );
            ledWall.storeInPanel( 0, row, col, panel );
            ledWall.storeInPanel( 0, row+1, col, panel );
            sleep_ms( true, 1 );
        }
